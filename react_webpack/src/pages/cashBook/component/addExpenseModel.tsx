...
//无状态组件
const AddModal: React.FC<CollectionCreateFormProps> = ({add_edit_expense_visible, onCreate, onAddCancel, cashBook,expensesTypeList}) => {
    const [form] = Form.useForm();

    return (
        <Fragment>
            <Modal visible={add_edit_expense_visible} title="Title" getContainer={false} width={750} onCancel={onAddCancel}
                ...
                <Form form={form} preserve={false} initialValues={{...cashBook,incomeOrExpense:'expense',postDate: moment(cashBook.postDate==''?new Date(): cashBook.postDate, dateFormat)}}>
                    ...
                    <Row gutter={24}>
                        ...
                        <Col span={4}>
                            <div className="ant-col ant-form-item-label">
                                <label htmlFor="dealNo" title="记账单号">记账单号</label>
                            </div>
                        </Col>
                        <Col span={8}>
                    ...
                    <Row gutter={24}>
                        <Col span={4}>
                            <div className="ant-col ant-form-item-label">
                                <label htmlFor="ieCode" className="ant-form-item-required" title="支出类型">支出类型</label>
                            </div>
                        </Col>
                        <Col span={8}>
                            <Form.Item name="ieCode" rules={[{required: true, message: '请选择支出类型!'}]}>
                                <Select placeholder="请选择支出类型">
                                    {
                                        expensesTypeList.map((item: BoxItemObj, index: number) => <Option key={index} value={item.boxCode}>{item.boxText}</Option>)
                                    }
                                </Select>
                            </Form.Item>
                        </Col>
                        <Col span={4}>
                            <div className="ant-col ant-form-item-label">
                                <label htmlFor="amt" title="支出金额">&nbsp;&nbsp;&nbsp;支出金额</label>
                            </div>
                        </Col>
                        <Col span={8}>
                            <Form.Item name="amt">
                                <InputNumber placeholder='支出金额' style={{width: 218}}/>
                            </Form.Item>
                        </Col>
                    </Row>
                    <Row gutter={24}>
